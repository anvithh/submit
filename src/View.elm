    ...
    let
        ...
        btn = 
                if shape == "rect" then
                            ...
                            , SA.fill clr
                            , SA.style "stroke-width:0.6;stroke:rgb(0,0,0)"
                            , SA.fillOpacity "0.5"
                            ]
                            []
...
viewDisplay ( x, y ) ( w, h ) str =
    ...
    S.g
        ...
        [ S.rect
            ...
            , SA.rx "1"
            , SA.style "fill:rgb(0 0 0);stroke-width:0.5;stroke:rgb(0,0,0)"
            , SA.fillOpacity "0"
            ]
            []
        , S.text_
...
getHeight buttons cols rows config = 
            case config.shape of
                ...
                "ellipse" ->
                    String.fromInt (60+ ((round config.ry) * 2) + ((rows - 1)*(((round config.ry) * 2)+rowgap)))
                _ ->
                    String.fromInt (56 + height + ((rows - 1)*(height+rowgap)))
...
{-
   ...
   2. The history
   3. The final answer
   4. All the buttonxs to be shown
...
viewCalc edits history answer buttons =
    ...
    S.svg
        ...
        , SA.height "80vh"
        , SA.style ("border-style:solid;padding:"++(String.fromInt config.frame_padding)++"px;")
        ]
